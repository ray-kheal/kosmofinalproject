...
public class home_recipeCommand implements PHJCommandImpl {
	...
	public void execute(Model model) {
		...
		/*
		 * 외부파일로 만든 파일에 저장된 게시판 페이징 설정값을 얻어온다. 초기상태는 String 형태이므로 int형으로 변환한 후 저장한다.
		 */
		int pageSize = 8;
		int blockPage = 4;
		...
		// 시작 및 끝 rownum 구하기
		int nowPage = req.getParameter("nowPage") == null || req.getParameter("nowPage") == "" ? 1
		...
		String pagingImg = PagingUtil.pagingImg(totalRecordCount, pageSize, blockPage, nowPage,
				req.getContextPath() + "/recipe.do?" + addQueryString);

		model.addAttribute("pagingImg", pagingImg);
		model.addAttribute("totalPage", totalPage);
		model.addAttribute("nowPage", nowPage);
		model.addAttribute("recipeRows", listRows);
