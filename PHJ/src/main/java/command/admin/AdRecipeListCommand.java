...
public class AdRecipeListCommand implements PHJCommandImpl {
	...
	public void execute(Model model) {
		...
		//출력할 리스트 가져오기
		ArrayList<recipeDTO> viewRow = dao.ad_list(paramMap);
		
		
		//가상번호 계산하여 부여하기
		int virtualNum =0;
		int countNum =0;
		for(recipeDTO row : viewRow) {
		...
		String pagingImg =PagingUtil.pagingImg(totalRecordCount,pageSize,blockPage, nowPage,
				req.getContextPath()+"/admin/pages/tables/recipeManagement.do?"+addQueryString);
		...
		System.out.println("recipe리스트커맨드 잘끝냄 ^^");
