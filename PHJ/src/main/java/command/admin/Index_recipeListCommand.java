...
public class Index_recipeListCommand implements PHJCommandImpl{
	...
	int recipe_totalRecordCount = dao.getTotalCount(paramMap);
	System.out.println("totalRecordCount:"+recipe_totalRecordCount);

	int pageSize = 5;
	int blockPage = 5;

	// 전체 페이지수 계산하기
	int totalPage = (int) Math.ceil((double) recipe_totalRecordCount / pageSize);
	...
	int virtualNum = 0;
	int countNum = 0;
	for(recipeDTO row: recipeviewRow)
	{
		// 가상번호 연산 후 setter를 통해 값을 저장함
		virtualNum = recipe_totalRecordCount - (((nowPage - 1) * pageSize) + countNum++);
