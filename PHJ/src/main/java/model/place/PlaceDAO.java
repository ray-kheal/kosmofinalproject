...
public class PlaceDAO {
	...
	public int getPlaceCount(double distance, double latTxt, double lngTxt) {
		...
		String query = "SELECT " + "		count(*) " + "	FROM " + "		phj_place " + "	WHERE "
				+ "		trunc(to_number(DISTNACE_WGS84(" + latTxt + "," + lngTxt + ",latitude, longitude))*10,5)<= "
				+ distance +
				// " AND ROWNUM BETWEEN 1 and 20 " +
				"	ORDER BY " + "		trunc(to_number(DISTNACE_WGS84(" + latTxt + "," + lngTxt
				+ ",latitude, longitude))*10,5) ASC ";
	...
	public ArrayList<PlaceDTO> list(Map<String, Object> map) {
		...
		String query = " SELECT * FROM( " + "    SELECT Tb.*, ROWNUM rNum FROM( " + "      SELECT * FROM PHJ_PLACE ";
	...
	public ArrayList<PlaceDTO> searchRadius(double distance, double latTxt, double lngTxt, int start, int end) {
		String query = " SELECT * FROM ( " + " SELECT tb.*, rownum rNum from (" + " SELECT "
				+ " place_code, place_name, place_name2, place_address, latitude, longitude, "
				+ " trunc(to_number(DISTNACE_WGS84(" + latTxt + "," + lngTxt
				+ ",latitude, longitude))*10,5) AS disKM, ROWNUM AS virtualNum " + " FROM phj_place "
				+ " WHERE trunc(to_number(DISTNACE_WGS84(" + latTxt + "," + lngTxt + ",latitude, longitude))*10,5)<= "
				+ distance + " ORDER BY disKM asc " + " ) tb " + " ) where rNum BETWEEN " + start + " AND " + end;
