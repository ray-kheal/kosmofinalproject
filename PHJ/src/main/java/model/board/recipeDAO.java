...
public class recipeDAO {
	...
	public ArrayList<recipeDTO> list(Map<String, Object> map) {
		...
		sql += " ORDER BY recommend DESC, postdate DESC ) tb) WHERE rNum BETWEEN " + start + " AND " + end
				+ " ORDER BY recommend DESC, postdate DESC";
	...
	public void write(final recipeDTO recipeDTO) {
		...
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				System.out.println("dao의 write - title:" + recipeDTO.getTitle() + "/content:" + recipeDTO.getContent()
						+ "email:" + recipeDTO.getEmail());
				return psmt;
			}
		});
	}
