...
public class noticeDAO {
	...
	public ArrayList<noticeDTO> popular_notice(Map<String, Object> map) {
		...
		String query = "  SELECT * FROM ( " + " SELECT * FROM PHJ_BOARD_NOTICE where board_type= " + board_type
				+ " order by view_count desc " + " ) WHERE ROWNUM <= 3 ";
	...
	public void write(final noticeDTO noticeDTO) {
		...
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				System.out.println("dao의 write - title:" + noticeDTO.getTitle() + "/contetn:" + noticeDTO.getContent()
						+ "/type:" + noticeDTO.getBoard_type());
				return psmt;
			}
		});
	}
